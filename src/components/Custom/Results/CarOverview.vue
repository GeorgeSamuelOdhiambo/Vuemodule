...
onMounted(async () => {
  await getCarInfo({ car: route.params.carId, selectedType: route.params.targetType })
})
...
watchEffect(async () => {
  ...
  else if (carStore.carInfo.carId)
    carIdValue.value = carStore.carInfo?.carId as string
...
watch(
  ...
  async () => {
    if(route.params.carId)
      await getCarInfo({
        car: route.params.carId as string,
...
<template>
  <div v-loading="carDataLoading">
    <div
      v-if="carStore.carInfo && carStore.carInfo.carId "
      class="w-full shadow-md rounded-md bg-white dark:bg-black px-10 transition ease-in-out delay-300"
    >
      <div class="flex flex-col md:flex-row justify-between">
        <div class="w-full md:w-full flex flex-col md:flex-row items-center gap-5">
          <VImg
            :width="200"
            :height="120"
            aspect-ratio="16/9"
          ...
          <VBtn
            ...
            append-icon="mdi-information-slab-circle-outline"
            @click="handleShowCarInfo"
          >
            {{ carStore.carInfo.TYPEL }}
          </VBtn>
        </div>
        <div class="w-full md:w-full flex items-center justify-center px-10">
          <VBtn
            variant="text"
            class="hover:bg-[#a9bdf1]"
            :prepend-icon="
              show && showType === 'select' ? 'mdi-chevron-up' : 'mdi-chevron-down'
            "
