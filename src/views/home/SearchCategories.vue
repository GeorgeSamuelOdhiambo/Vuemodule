...
const handleLevel3Click = (item: ICategory) => {
  if (item.STR_LEVEL === 3) {
    ...
    query_params.value.node_2_id = item.NODE_2_STR_ID
    router.push({
      name: 'Categories',
      params: {
        carId: route.params.id,
        treeName: item.ROOT_NODE_TEXT,
        strName: item.NODE_3_TEXT,
...
const handlePrimaryCategoryClick = (values: ICategory[]) => {
  ...
  console.log(values)
  query_params.value.root_node_id = values[0].ROOT_NODE_STR_ID
  router.push({
    name: 'Categories',
    params: {
      carId: route.params.id,
      treeName: values[0].ROOT_NODE_TEXT,
    },
    query: { ...query_params.value, ...{ filtercat: route.query.filtercat } },
  })
}
...
const handleNestedCartegoriesClick = (categoryItem: ICategory) => {
  ...
  if (categoryItem.STR_LEVEL === 2) {
    ...
    query_params.value.node_1_id = categoryItem.NODE_1_STR_ID
    router.push({
      name: 'Categories',
      params: {
        carId: route.params.id,
        treeName:
      categoryItem.ROOT_NODE_TEXT,
        strName: categoryItem.NODE_2_TEXT,
        strid: categoryItem.NODE_1_STR_ID,
      },
      query: { ...query_params.value, ...{ filtercat: route.query.filtercat } },
    })
  }
